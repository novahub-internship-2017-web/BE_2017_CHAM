...
<sql:setDataSource var="snapshot" driver="com.mysql.jdbc.Driver"
     url="jdbc:mysql://localhost/QuanLyNhanVien"
...
<%
	if(request.getAttribute("listTeacher") != null){
		...
		for(Teacher objTeacher : listTeacher){
			int i=0;
%>
<tr>
   <th><%=i++ %></th>
   <th><%=objTeacher.getName() %></th>
   <th><%=objTeacher.getBirthDay() %></th>
   <th><%=objTeacher.getCountry() %></th>
   <th>Giảng Viên</th>
   <th><%=objTeacher.getFacutly() %></th>
   <th><%=objTeacher.getDegree() %></th>
   <th><%=objTeacher.getAllowance() %></th>
   <th><%=objTeacher.getLessons() %></th>
   <th><%=objTeacher.getCoefficientSalary() %></th>
   <th><%=objTeacher.getSalary() %></th>
   <th></th>
</tr>
<%} }%>
</table>
